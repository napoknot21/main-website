...
export function TechnologyPage() {
  ...
  return <PageShell>
    ...
    <Section title="Technology at the core"><div className="grid gap-5 md:grid-cols-2"><Card title="Heroics Aegis"><div className="relative w-full aspect-video mb-4 rounded-lg overflow-hidden bg-deepblue"><video className="w-full h-full object-cover" controls><source src="/videos/aegis_video.mp4" type="video/mp4" /></video></div><p className="text-sm leading-relaxed text-muted-foreground">{s("Trade lifecycle, booking, compliance workflows, regulatory controls and portfolio operations in one proprietary environment.")}</p></Card><Card title="Heroics Sentinelle"><div className="relative w-full aspect-video mb-4 rounded-lg overflow-hidden bg-deepblue"><video className="w-full h-full object-cover" controls><source src="/videos/sentinelle_video.mp4" type="video/mp4" /></video></div><p className="text-sm leading-relaxed text-muted-foreground">{s("Risk monitoring, portfolio analytics, scenario analysis, dashboards and limit surveillance across mandates and fund structures.")}</p></Card></div></Section>
  </PageShell>
}
